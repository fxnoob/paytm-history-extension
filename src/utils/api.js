...
export class Api {
  ...
  async fetchTxHistory() {
    let apiEndPoint = this.getTxHistoryEndPoint(10, this.totalTxHistoryApiCalls);
    return fetch(apiEndPoint)
      .then(res=>{
        if(res.status === 200) {
          return res;
      ...
      .then(res => {
        ...
        if(res.response.length !== 0) {
          return this.fetchTxHistory(this.getTxHistoryEndPoint(10, this.totalTxHistoryApiCalls));
        } else {
          return "DONE";
        }
      })
